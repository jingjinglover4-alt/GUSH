...
def public_claim_submit(request):
    ...
    # 每日窗口限领检查：同 openid + 同设备 + 当天窗口内已用码，则拒绝发新码
    if machine_id:
        machine_obj = Machine.objects.filter(machine_id=machine_id).first()
        if machine_obj and user_already_redeemed_today(project, openid, machine_obj):
            return Response(
                {"detail": "今天已在此设备兑换过，请明天再来"},
                status=429,
            )
...
def public_redeem_submit(request):
    ...
    with transaction.atomic():
        ...
        # 每日窗口限领检查
        if user_already_redeemed_today(code.project, code.user_openid, machine):
            return Response(
                {"detail": "今天已在此设备兑换过，请明天再来"},
                status=429,
            )
...
def qt_exchange(request):
    """
    Qt 上位机兑换码验证接口（适配 Qt 端 mainwindow.cpp 的判定）

    请求: {"code": "XXXX", "machine_id": "MACHINE001"}
    返回（Qt 判定分支见 mainwindow.cpp onExchangeResult）:
        成功:  {"type": "random"|"order", "result": "on",   "code": "..."}
        失败:  {"type": "random"|"order", "result": "<原因>"}
    result 取值（必须与 Qt 端一致）:
        on              - 兑换码有效，Qt 端开始出货
        invalid         - 兑换码不存在 / 已作废 / 与设备不匹配
        used            - 兑换码已使用
        expired         - 兑换码已过期
        out_of_stock    - 设备无可用库存（Qt 显示"商品已售馨"）
        off             - 兜底（参数错误/设备不存在）

    流程：
      1. 校验兑换码状态（不存在/已用/已作废/已过期/不匹配）
      2. 选一个可用货道（库存>0 且非故障；优先项目礼品）
      3. 标记 RedeemCode 为 USED
      4. 预创建 DispenseLog(result=BUSY) 等待 qt_report 来结算
      5. 返回 type 给 Qt，由 Qt 端按 random/order 策略自己选货道出货
    """
    ...
    with transaction.atomic():
        ...
        if not code:
            return Response({"type": qt_type, "result": "invalid"})
        if code.status == RedeemCode.Status.USED:
            return Response({"type": qt_type, "result": "used"})
        if code.status == RedeemCode.Status.REVOKED:
            return Response({"type": qt_type, "result": "invalid"})
        if code.expires_at and code.expires_at < timezone.now():
            if code.status != RedeemCode.Status.EXPIRED:
                code.status = RedeemCode.Status.EXPIRED
                code.save(update_fields=["status"])
            return Response({"type": qt_type, "result": "expired"})
        if code.status != RedeemCode.Status.UNUSED:
            return Response({"type": qt_type, "result": "invalid"})
        ...
        # 每日窗口限领检查：同一 openid + 同一设备 + 当天窗口内已成功兑换则拒绝
        # Qt 端没有 openid 输入，靠 code.user_openid 识别用户
        if user_already_redeemed_today(code.project, code.user_openid, machine):
            return Response({"type": qt_type, "result": "invalid"})

        # 选一个可用货道（只校验是否有库存，不在这里扣库存）
        ch_qs = (
            Channel.objects
            .filter(machine=machine, current_stock__gt=0)
            .exclude(status=Channel.Status.FAULT)
        ...
        scoped = ch_qs
        if project_products:
            scoped = ch_qs.filter(product_id__in=project_products)
        if not scoped.exists():
            scoped = ch_qs
        if not scoped.exists():
            return Response({"type": qt_type, "result": "out_of_stock"})

        # 标记为已使用
        code.status = RedeemCode.Status.USED
        code.used_at = timezone.now()
        code.used_on_machine = machine
        # used_on_channel_code 留给 qt_report 来填
        code.save(update_fields=["status", "used_at", "used_on_machine"])

        # 预建 BUSY 日志：qt_report 来时根据这个找到并结算
        DispenseLog.objects.create(
            ...
            detail="Qt 兑换码验证通过，等待设备出货上报",
        )

    return Response({"type": qt_type, "result": "on", "code": code_str})
@csrf_exempt
@api_view(["POST"])
@permission_classes([AllowAny])
def qt_report(request):
    ...
    返回: {"ok": true, "channel": "A0"}

    行为：
      - 找到该机器最近一条 BUSY 日志（含 redeem_code 关联）→ 改 SUCCESS
      - 扣 Channel.current_stock，刷新状态
      - 写 StockMovement 流水（与 ws/device 的 motor_done 保持同一通道）
      - 回填 RedeemCode.used_on_channel_code
      - 通过 WS 推 stock_update 给该机器（其它监听者刷新）
    """
    channel_code = (request.data.get("channel", "") or "").strip().upper()
    machine_id = (request.data.get("machine_id", "") or "").strip()

    if not channel_code or not machine_id:
        return Response({"ok": False, "detail": "缺少 channel 或 machine_id"})

    machine = Machine.objects.filter(machine_id=machine_id).first()
    if not machine:
        return Response({"ok": False, "detail": "设备不存在"})

    # Qt 端编号 1..60 也兼容（万一 Qt 端自己改了）
    # 但约定：Qt 上报的是 channel_name（如 "A0"）；如果传的是数字则反解
    if len(channel_code) == 1 or not channel_code[0].isalpha():
        # 数字 → 反解为通道名
        try:
            motor_id = int(channel_code)
        except ValueError:
            return Response({"ok": False, "detail": "channel 格式错误"})
        if not (1 <= motor_id <= 60):
            return Response({"ok": False, "detail": "channel 越界"})
        idx = motor_id - 1
        row_idx, col = divmod(idx, 10)
        channel_code = f"{chr(ord(A) + row_idx)}{col}"
    ...
    with transaction.atomic():
        ch = (
            ...
            .filter(machine=machine, channel_code=channel_code)
            .first()
        )
        if not ch:
            return Response({"ok": False, "detail": "货道不存在"})

        # 找该机器上最近一条 BUSY 日志（含 redeem_code）
        pending = (
            DispenseLog.objects
            .select_for_update()
            .filter(machine=machine, result=DispenseLog.Result.BUSY)
            .order_by("-created_at")
            .first()
        )
        ...
        if pending and pending.redeem_code_id:
            ...
            pending.channel = ch
            pending.channel_code = channel_code
            pending.detail = "Qt 上报出货完成"
            pending.save(update_fields=["result", "channel", "channel_code", "detail"])
            ...
            log = DispenseLog.objects.create(
                ...
                detail="Qt 上报出货完成（无关联兑换码）",
            )

        # 扣库存
        if ch.current_stock > 0:
            ch.current_stock -= 1
            ch.last_dispense_at = timezone.now()
            ch.save(update_fields=["current_stock", "last_dispense_at", "updated_at"])
        ch.recompute_status(save=True)
        ch.refresh_from_db()

        # 回填 redeem_code.used_on_channel_code
        if redeem and not redeem.used_on_channel_code:
        ...
        try:
            ...
            stock_services.record_dispense(
                ...
                channel=ch,
                quantity=1,
                channel_stock_after=ch.current_stock,
                redeem_code=redeem,
                dispense_log=log,
                source="qt_report",
                note="Qt 上位机上报出货完成",
            ...
            # 流水写失败不影响主流程
            import logging
            logging.getLogger("apps.projects.views").warning(
                "record_dispense failed for qt_report: %s", e
            )
    ...
    try:
            ...
            async_to_sync(layer.group_send)(
                f"device_{machine.machine_id}",
                {
                    ...
                    "payload": {
                        "cmd": "stock_update",
                        "channels": [
                            {"code": ch.channel_code, "stock": ch.current_stock},
                        ],
                    },
                },
            )
        ...
        import logging
        logging.getLogger("apps.projects.views").warning(
            "stock_update push failed for qt_report: %s", e
        )

    return Response({"ok": True, "channel": channel_code, "stock": ch.current_stock})


@csrf_exempt
@api_view(["GET"])
@permission_classes([AllowAny])
def qt_channels(request):
    """
    Qt 上位机拉取货道库存
    请求: GET /api/public/redeem/api/channels/?machine_id=X&secret=Y
    返回: {"channels": [{"code":"A0","stock":5,"status":"normal"}, ...]}
    """
    machine_id = (request.GET.get("machine_id", "") or "").strip()
    secret = (request.GET.get("secret", "") or "").strip()
    if not machine_id or not secret:
        return Response({"detail": "缺少 machine_id 或 secret"}, status=400)

    machine = Machine.objects.filter(machine_id=machine_id).first()
    if not machine:
        return Response({"detail": "设备不存在"}, status=404)
    if machine.comm_secret != secret:
        return Response({"detail": "鉴权失败"}, status=403)

    channels = (
        Channel.objects
        .filter(machine=machine)
        .order_by("row", "col")
        .values("channel_code", "current_stock", "status", "capacity")
    )
